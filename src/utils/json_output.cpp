...
void output_result(std::ostream& os, const core::ETLResult& result, bool json_mode, bool verbose) {
    if (json_mode) {
...
void output_results(std::ostream& os, const std::vector<core::ETLResult>& results, bool json_mode, bool verbose) {
    if (json_mode) {
        ...
        for (const auto& r : results) {
            switch (r.status) {
                case core::ResultStatus::Success: success_count++; break;
                case core::ResultStatus::Error: error_count++; break;
                case core::ResultStatus::Skipped: skipped_count++; break;
            }
        }
    ...
    if (results.size() > 1) {
        ...
        for (const auto& r : results) {
            if (r.status == core::ResultStatus::Success) success_count++;
            else if (r.status == core::ResultStatus::Error) error_count++;
        }
...
void output_plugin_info(std::ostream& os, const core::PluginInfo& info, bool json_mode) {
    ...
    os << info.group << " (" << core::etl_type_to_string(info.etl) << ")";
    if (info.builtin) {
        os << " [built-in]";
    }
    os << std::endl;
...
void output_preset(std::ostream& os, const core::Preset& preset, bool json_mode) {
    ...
    os << "ETL: " << core::etl_type_to_string(preset.etl) << std::endl;
...
void output_presets(std::ostream& os, const std::vector<core::Preset>& presets, bool json_mode) {
    ...
    for (const auto& preset : presets) {
        ...
        os << " [" << core::etl_type_to_string(preset.etl) << " → " << preset.target << "]";
        os << std::endl;
    }
}
